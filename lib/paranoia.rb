...
class ActiveRecord::Base
  def self.acts_as_paranoid(options={})
    ...
    default_scope { where(self.quoted_table_name + ".#{paranoia_column} IS NULL") } if options.fetch(:default_scope, true)

    scope :without_deleted, ->{ where(self.quoted_table_name + ".#{paranoia_column} IS NULL") }
